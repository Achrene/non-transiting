...
model_grid_folder = '/Users/adyrek/PycharmProjects/punto/results_grid/model_grid/full_1_62_inclination'
main_result_folder= '/Users/adyrek/PycharmProjects/punto/results_grid/ultranest/fit_fixed_i'
if fit_gravitational_effects is True:
    result_folder = os.path.join(main_result_folder, 'fitted_grav_params')
else:
    if add_gravitational_effects_model is True:
        result_folder = os.path.join(main_result_folder, 'fixed_grav_params')
    else:
        result_folder = os.path.join(main_result_folder, 'default')
...
if create_grid is True:
    ...
    def run_model(planetaryradius, albedo, redistribution):
        punto_system = ExoplanetarySystem.ExoplanetarySystem(orbitalperiod=period,
...
if plot_grid is True:
    ...
    def plot_photometry_model(normalized_flux, planetaryradius, albedo, redistribution):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(model_grid_folder, f'{model_name}.png'),  format='png', dpi=300, bbox_inches='tight')
        plt.show()
...
if perform_fit is True:
    ...
    if fit_gravitational_effects is False:
        ...
        param_names = ["Planetary Radius", "Albedo", "Redistribution", "Abeam", "Aellip", "Offset"]
        param_bounds = [
            (planetaryradius.min(), planetaryradius.max()),
            (albedo.min(), albedo.max()),
            (redistribution.min(), redistribution.max()),
        ...
        def prior_transform(unit_cube):
            ...
            for i, (u, (low, high)) in enumerate(zip(unit_cube, param_bounds)):
                if param_names[i] in ["Abeam", "Aellip"]:
                    ...
                    val = 10 ** (np.log10(max(low, 1e-6)) + u[i] * (np.log10(max(high, 1e-6)) - np.log10(max(low, 1e-6))))
