...
input_folder = sys.argv[1]  # '/Users/ah258874/Documents/Thèse/Punto
output_location = sys.argv[2]  # '/Users/ah258874/Documents/Thèse/Punto/test'
...
def phase_curve_model(parameters, time, period, T0, M_star, R_star, sini, Teff):
    ...
    # coeff_beam = alpha_beam
    # coeff_refl = alpha_refl * np.sin(i) * (Rp/R_jup)**2 where Rp is the planet radius and i the orbit inclination
    ...
    # Aellip = 13 * alpha_ellip * sini * R_star ** 3 * M_star ** (-2) * (period / 1) ** (-2) * Mp_sini
    # Lillo-Box, 2021
    # Abeam = 2.7 * alpha_beam * (period / 1) ** (-1 / 3) * M_star ** (-2 / 3) * Mp_sini  # Lillo-Box, 2021

    F = A0 - Arefl * np.cos(2 * np.pi * (phi + delta)) - Aellip * np.cos(4 * np.pi * phi) + Abeam * np.sin(2 * np.pi * phi)
...
if __name__ == '__main__':
    ...
    rv_model_value = rv_model(Ks, t_rv, period, T0_phase, M_star)
    ...
    parameters_phase = [10, 10, 10, -1, -1]
    initial = parameters_phase

    bounds = Bounds([1 * 10**(-3), 1 * 10**(-3), 1 * 10**(-3),  -np.inf, -np.inf], [+np.inf, +np.inf, +np.inf, 0, +np.inf])
    ...
    joint_parameters = np.array((Ks[0], params_mle[0], params_mle[1], params_mle[2], params_mle[3], params_mle[4]))
    ...
    pos = joint_parameters + 1e-4 * np.random.randn(100, 6)
    ...
    fig, axes = plt.subplots(6, figsize=(5, 4), sharex=True)
    labels = ["K", "Arefl", "Aellip", "Abeam", "A0", "Delta"]
    for i in range(ndim):
        ax = axes[i]
    ...
    for ind in inds:
        ...
        best_fit_phase = phase_curve_model(sample[1:], x_phase, period, T0_phase, M_star, R_star, sini, Teff)
