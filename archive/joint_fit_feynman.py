...
input_folder = '/Users/ah258874/Documents/Thèse/Punto' #sys.argv[0]
output_location =   '/Users/ah258874/Documents/Thèse/Punto/test' #sys.argv[1]
...
def rv_model(parameters, t, P, T0, Mstar):
    ...
    f = (2 * np.pi / P) * (t - T0)  # true anomaly for circular orbits
...
def phase_curve_model(parameters, time, period, T0, M_star, R_star, sini, Teff):
    ...
    # b = 0.29  # 0.63
    # Rp_sini = 1.03 * (Mp_sini * M_jup) ** b  # b = 0.29 for mean density rho > 3.3 g cm-3 from J. F. Otegi+,  2020
    # ROCKY
    # Rp_sini = 0.7 * (Mp_sini * M_jup) ** b - b = 0.63 for mean density rho < 3.3 g cm-3 from J. F. Otegi+,
    # 2020 #GASEOUS OR WATER WORLDS

    # Rp = Rp_sini / sini  # in absolute mass
    # print("Rp", Rp/R_jup)

    # K = 28.4 * ((period / 365) / 1)**(-1/3) * Mp_sini * M_star**(-2/3)  # Torres et al., 2008; 28.4 in m/s
    # print("K", K)

    alpha_beam = -6 * (10 ** -4) * Teff + 7.2  # Kepler bandpass, Lillo-Box, 2021 and Millholland & Laughlin 2017
    # print("alpha_beam", alpha_beam)
    # alpha_ellip = -2.2 * (10 ** -4) * Teff + 2.6  # Kepler bandpass, Lillo-Box, 2021 and Millholland & Laughlin 2017
    # print("alpha_ellip", alpha_ellip)
...
def log_probability(parameters, X, Y, Z, period, T0_phase, T0_rv, M_star, R_star, sini, Teff):
    x_phase = X[0]
    y_phase = Y[0]
    yerr_phase = Z[0]
    x_rv = X[1]
    ...
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood_phase(parameters, x_phase, y_phase, yerr_phase, period, T0_phase, M_star, R_star, sini,
                                     Teff) + log_likelihood_rv(parameters, x_rv, y_rv, yerr_rv, period, T0_rv, M_star)
...
def phase_fold(x, y, period):
    df = pd.DataFrame(np.column_stack((x, y)), columns=['t', 'f'])

    t0 = df['t'][0]
    df['p'] = (df['t'] - t0 + 0.5 * period) % period - 0.5 * period
...
if __name__ == '__main__':
    ...
    parameters_phase = [Ms_sini[0], 0.5, 1, 1]
    joint_parameters = [Ms_sini[0], 0.5, 1, 1]
    ...
    print(f"Used parameters in the right order: "
          ...
          f"A0 amplitude"
          f"Phase shift")
    print(f"Parameters first guess : {parameters_phase}")

    t_rv = np.linspace(x_rv.min(), x_rv.max(), 10000)
    rv_model_value = rv_model(joint_parameters, t_rv, period, T0_rv, M_star)

    fold_t_rv, fold_rv_model = phase_fold(t_rv, rv_model_value, period)
    fold_x_rv, fold_y_rv = phase_fold(x_rv, y_rv, period)
    ...
    bounds = Bounds([0, 0, -np.inf, -np.inf], [+np.inf, +np.inf, +np.inf, +np.inf])
    ...
    fold_x_phase, fold_y_phase = phase_fold(x_phase, y_phase, period)
    phase_curve_model_value = phase_curve_model(params_mle, x_phase, period, T0_phase, M_star, R_star, sini, Teff)
    fold_x_phase, fold_phase_curve_model = phase_fold(x_phase, phase_curve_model_value, period)
    ...
    pos = np.array((Ms_sini[0], params_mle[1], params_mle[2], params_mle[3])) + 1e-4 * np.random.randn(100, 4)
    ...
    with Pool() as pool:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, log_probability,
                                        args=(X, Y, Z, period, T0_phase, T0_rv, M_star, R_star, sini, Teff), pool=pool)
        start = time.time()
        max_n = 100000  # 300000
    ...
    fig, axes = plt.subplots(4, figsize=(5, 4), sharex=True)
    labels = ["K", "A0", "Arefl", "Delta"]
    for i in range(ndim):
        ax = axes[i]
    ...
    for ind in inds:
        ...
        fold_x_phase, fold_best_fit_phase = phase_fold(x_phase, best_fit_phase, period)
    ...
    for ind in inds:
        sample = flat_samples[ind]
        best_fit_rv = rv_model(sample, x_rv, period, T0_rv, M_star)
        fold_x_rv, fold_best_fit_rv = phase_fold(x_rv, best_fit_rv, period)
